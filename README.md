...
| `ops/test-bootstrap-jankurai-root-seal.sh` | Proves wrong digest/ref/head/tree/receipt, expiry, head/attempt reuse, caller command, pathname replacement, same-inode content drift, held execution, signal, abrupt-death recovery, failure-result, and byte-exact restoration behavior. |
